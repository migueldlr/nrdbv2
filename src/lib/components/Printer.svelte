<script lang="ts">
	...
	const nsg_cards = $derived(
		cards.filter((card) => card.attributes.designed_by.includes('null_signal_games'))
	);

	const cards_formatted = $derived<Card[]>([
		// Place identity first
		...nsg_cards.filter((card) => card.attributes.card_type_id.endsWith('_identity')),
...
<div class="printer">
	{#each cards_formatted as card (card.id)}
		...
		{#each Array(decklist.attributes.card_slots[card.id]) as _, index (index)}
			...
			<div
				class="printer__card"
				style="--image: url({getHighResImage(
					card,
					'xlarge'
