	...
	{:else}
		<div class="picker__groups">
			{#each visibleFactions as group (group.faction_id)}
				<section class="picker__group" style={`--color: var(--${group.faction_id})`}>
					<h3 class="picker__group-title">
						<Icon name={group.faction_id} theme={group.faction_id} />
						{group.name}
						({group.cards.length})
					</h3>
						...
						{#each group.cards as identity (identity.id)}
							<li>
								<button
									...
									onclick={() => decklistNav.selectIdentity(identity.id)}
								>
									<CardImage card={identity} href={null} boxShadow={false} />
