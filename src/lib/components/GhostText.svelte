<script lang="ts">
	...
	const wordEnd = $derived(hasSelection ? -1 : getWordRangeAtCursor(value, caretPosition).end);
...
<div class="ghost-input">
	...
	{#if remainder}
		<!-- Example: typing "jin" with the suggestion "teki" draws an invisible
             ...
             line reads "jinteki" -->
		<span class="ghost" aria-hidden="true" style="--scroll: {-scrollLeft}px">
			<span class="ghost-typed">{typed}</span><span class="ghost-remainder">{remainder}</span>
		</span>
	{/if}
</div>
